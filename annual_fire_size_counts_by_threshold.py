...
def count_fire_sizes_annual_historical( fn, threshold=4 ):
	''' 
	ARGUMENTS:
	----------
	fn = [str] path to the ALFRESCO Post Processing output JSON file
	...
	for domain in domains:
		...
		# make a dataframe for the domain
		columns = ['below_{}_{}'.format( threshold, domain ), 'above_{}_{}'.format( threshold, domain ) ]
		df = pd.DataFrame(list(zip( below, above )), columns=columns, index=years )
		# panel = df.to_panel() # make it wide-format
		# concat replicates to single DataFrame
		# domain_df = pd.concat([ _update_cols( panel[ :, repnum, : ], repnum ) for repnum in np.unique( replicates ) ], axis=1 )
		dd[ domain ] = df
...
if __name__ == '__main__':
	...
	for fn in glob.glob(os.path.join(json_path, '*.json')):
		print( fn )
		model, scenario = os.path.basename(fn).split('.')[0].split('_')
		
		# run it
		if 'historical' in fn or 'observed' in fn:
			df_dict = count_fire_sizes_annual_historical( fn, threshold=threshold )
		else:
		...
		for domain, df in df_dict.items():
			...
			output_filename = os.path.join( output_path, 'annual_firesize_counts_{}_{}_threshold{}_{}.csv'.format(model, scenario, threshold, domain) )
